...
X_true, y_true = qg_jets.load(train_true + val_true + test_true, generator='pythia')
X_true = tf.cast(X_true, tf.float32)
y_true = tf.cast(y_true, tf.float32)

#X_true, y_true = qg_jets.load(train_true + val_true + test_true, generator='herwig')
n_true_pad = 148 - X_true.shape[1]
...
########## Parameters
# dataset
m         = 148       # particles per jet, note above has a padding step, be sure to change that also
...
opt_g      = keras.optimizers.Adam(5e-5, beta_1=0.5) #1e-4
opt_d      = keras.optimizers.Adam(5e-5, beta_1=0.5) #1e-4
...
save_dir = "/users/yzhou276/work/dataset_distillation/qgtag/distill/"
...
def fwd_dense(x, w, dense_sizes, activations, drop_rates, training=True, seed=None):
    ...
    k_out = tf.cast(k_out, dtype=tf.float32)
    b_out = tf.cast(b_out, dtype=tf.float32)
...
#noise = tf.random.normal((1, latent_dim), dtype=tf.float32)
